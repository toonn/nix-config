        ...
        options = {
          ...
          randomizedDelaySec = mkOption {
            ...
            description = lib.mdDoc ''
              ...
              {manpage}`systemd.time(7)`. Either a unit-less value (in seconds)
              or a time span value, e.g., `"5min 20s"`.
            '';
            example = "5 min";
          };
          ...
          restartSec = mkOption {
            ...
            default = null;
            description = lib.mdDoc ''
              Whether and how soon a failed backup should be restarted. Must be
              in the format described in {manpage}`systemd.time(7)`. Either a
              unit-less value (in seconds) or a time span value, e.g.,
              `"5min 20s"`. If you do not want a failed backup to restart
              automatically, use `null`.
            '';
            example = "10 min";
          };
